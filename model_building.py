...
### KNN-based
# convert data to usable format for Surprise library
...
# use anti_test set to get user-item pairs that do not have ratings
...
def remove_rated_titles(user, user_recs, n_recs):
...
def svd_based_recs(user, n_recs):
    ...
    # # next sort the predictions by user
    # for uid, user_ratings in preds.items():
    #     user_ratings.sort(key=lambda x: x[1], reverse=True)
    #     preds[uid] = user_ratings

    # examine top n movies for user
